...
# ============================================================================
# MEALS  /users/{user_id}/meals
# ============================================================================

@router.post("/{user_id}/meals", status_code=status.HTTP_201_CREATED)
def add_meal(
    user_id: str,
    body: MealCreate,
...
def get_all_meals(
    ...
    If no dates provided, returns all meals.
    """
    username = backend.get_username_from_session()
...
@router.get("/{user_id}/meals/date/{date_string}")
...
@router.get("/{user_id}/meals/{meal_id}")
def get_meal(
    user_id: str,
    meal_id: int,
    backend: FitnessBackend = Depends(get_backend)
...
@router.patch("/{user_id}/meals/{meal_id}")
def update_meal(
    user_id: str,
    meal_id: int,
    body: MealUpdate,
...
@router.delete("/{user_id}/meals/{meal_id}", status_code=status.HTTP_204_NO_CONTENT)
def delete_meal(
    user_id: str,
    meal_id: int,
    backend: FitnessBackend = Depends(get_backend)
...
# ============================================================================
# FOOD  /users/{user_id}/meals/{meal_id}/food
# ============================================================================

@router.post("/{user_id}/meals/{meal_id}/food", status_code=status.HTTP_201_CREATED)
def add_food(
    user_id: str,
    meal_id: int,
    body: FoodCreate,
...
@router.get("/{user_id}/meals/{meal_id}/food")
def get_food_by_meal(
    user_id: str,
    meal_id: int,
    backend: FitnessBackend = Depends(get_backend)
):
...
@router.get("/{user_id}/meals/{meal_id}/food/{food_id}")
def get_food(
    user_id: str,
    meal_id: int,
    food_id: int,
...
@router.patch("/{user_id}/meals/{meal_id}/food/{food_id}")
def update_food(
    user_id: str,
    meal_id: int,
    food_id: int,
...
@router.delete("/{user_id}/meals/{meal_id}/food/{food_id}", status_code=status.HTTP_204_NO_CONTENT)
def delete_food(
    user_id: str,
    meal_id: int,
    food_id: int,
...
# ============================================================================
# NUTRITION ANALYTICS  /users/{user_id}/meals/analytics/...
# ============================================================================

@router.get("/{user_id}/meals/analytics/summary")
...
@router.get("/{user_id}/meals/analytics/calories-consumed")
...
try:
    ...
    @router.get("/{user_id}/meals/food-search")
    def search_food(
        user_id: str,
    ...
    @router.get("/{user_id}/meals/food-search/barcode/{barcode}")
    ...
    class SaveFoodByBarcodeRequest(PydanticBaseModel):
        barcode: str
        food_type: str = Field(default="Other")
        servings: float = Field(default=1.0, gt=0, le=50)

    class SaveFoodByNameRequest(PydanticBaseModel):
        name: str
        calories: int = Field(ge=0, le=10000)
        food_type: str = Field(default="Other")

    @router.post("/{user_id}/meals/{meal_id}/food-search/save-by-barcode", status_code=status.HTTP_201_CREATED)
    def save_food_by_barcode(
        ...
        Typical flow: scan barcode → call this endpoint → food saved.
        """
        try:
            result = client.get_product_by_barcode(
                body.barcode,
    ...
    @router.post("/{user_id}/meals/{meal_id}/food-search/save-by-name", status_code=status.HTTP_201_CREATED)
    def save_food_by_name(
        user_id: str,
        meal_id: int,
        body: SaveFoodByNameRequest,
        backend: FitnessBackend = Depends(get_backend)
    ):
        """
        After searching, user picks a result — pass name + calories to log it to a meal.
        Typical flow: search food → pick result → call this endpoint → food saved.
        """
        try:
            return backend.add_food_to_meal(meal_id, body.name, body.food_type, body.calories)
        except Exception as e:
