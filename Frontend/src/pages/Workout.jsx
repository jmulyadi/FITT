  ...
  return (
    <div className="page fade-in">
      ...
      <div className="page-header">
        <div style={{ flex: 1 }}>
          <div style={{ fontSize: 12, color: 'var(--muted)', marginBottom: 2 }}>Upper Body – Push</div>
      ...
      <div className="page-content">
        ...
        {saved && (
          <div className="finish-banner fade-in">
            <h3>Session Saved</h3>
            <p>Volume: 12,450 lbs (+8%). Bench press velocity improved. <strong>Increase bench by 5 lbs next session.</strong></p>
          </div>
        )}
        ...
        <div className="rest-timer">
          <div>
            ...
            <div style={{ fontSize: 11, color: 'var(--muted)' }}>90s recommended</div>
          </div>
          <div style={{ textAlign: 'right' }}>
            {/* Timer turns green when it hits 0 to signal rest is done */}
            <div className="timer-val" style={{ color: timerSec <= 0 ? 'var(--green)' : 'var(--text)' }}>
              {formatTime(timerSec)}
            </div>
            <button
        ...
        {exercises.map((ex, exIdx) => (
          <div key={ex.name} className="exercise-card fade-in">
            <div className="ex-header">
              <div>
                ...
                <div style={{ marginTop: 3, display: 'flex', gap: 6 }}>
                  {ex.muscles.map(m => <span key={m} className="muscle-tag">{m}</span>)}
                </div>
              </div>
            </div>
            ...
            <div className="sets-header">
              <span>#</span><span>Weight (lbs)</span><span>Reps</span><span>RPE</span>
            </div>
            ...
            {ex.sets.map((s, setIdx) => {
              ...
              return (
                <div key={setIdx} className="set-row">
                  ...
                  <div className={`set-num${empty ? ' empty' : ''}`}>{setIdx + 1}</div>

                  {/* Controlled inputs — each change calls updateSet */}
                  <input className="set-input" value={s.weight} placeholder="—"
                    onChange={e => updateSet(exIdx, setIdx, 'weight', e.target.value)} />
                  <input className="set-input" value={s.reps} placeholder="—"
                    onChange={e => updateSet(exIdx, setIdx, 'reps', e.target.value)} />
                  <input className="set-input" value={s.rpe} placeholder="—"
        ...
        <div style={{ padding: '0 16px', display: 'flex', gap: 10, marginBottom: 14 }}>
          ...
          <button className="btn btn-ghost" style={{ flex: 1 }}
            onClick={() => alert('Exercise catalog search — coming soon!')}>
            + Add Exercise
          </button>
          {/* Opens the post-workout summary modal */}
          <button className="btn btn-green" style={{ flex: 1 }} onClick={() => setShowModal(true)}>
            Finish Workout
          </button>
        </div>
      </div>
